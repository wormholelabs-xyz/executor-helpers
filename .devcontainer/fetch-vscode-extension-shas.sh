...
process_ext streetsidesoftware code-spell-checker   SPELLCHECKER_EXT      & PIDS+=($!)
process_ext bierner           markdown-mermaid      MARKDOWN_MERMAID_EXT  & PIDS+=($!)
process_ext rust-lang         rust-analyzer         RUST_ANALYZER_EXT     & PIDS+=($!)
process_ext tamasfe           even-better-toml      EVEN_BETTER_TOML_EXT  & PIDS+=($!)
process_ext JuanBlanco        solidity              SOLIDITY_EXT          & PIDS+=($!)
